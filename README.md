...
![Project pipeline](assets/fig_17_project_pipeline.png)

*Figure: high-level flow from raw inputs to metrics, backtests, and selection (generated by `news_impact.generate_report_figures`).*
...
The main neural model is a **feed-forward MLP** in PyTorch:

![MLP architecture](assets/fig_16_mlp_architecture_diagram.png)

*Figure: MLP layout — Linear → ReLU → Dropout × 2 hidden blocks → multi-output head (from `modeling.py` + trained manifest).*
...
| Figure | Caption |
|--------|---------|
| Pipeline | End-to-end module flow |
| MLP architecture | Network diagram from code + manifest |
